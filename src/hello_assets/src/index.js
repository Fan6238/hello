...
$("#followsList").on("click","p", function() {

    let principal = $(this).find("span").text();
...
async function follows() {
    ...
    try {
        ...
        for (let i = follows.length - 1; i >= 0; i --) {
            ...
            $("#followsList").append("<p>Pid : <span>" + follows[i] + "</span>;&nbsp;&nbsp;&nbsp;&nbsp;Name : " + name + "</p>");
        }
    } catch(err) {
        console.log(err);
...
async function posts() {
    ...
    try {
        ...
        for (let i = posts.length - 1; i >= 0; i --) {
            let html = '<span style="font-size:20px;">' + posts[i].text + "</span><br>Author : " + posts[i].author + "&nbsp;&nbsp;&nbsp;&nbsp;Time : " 
                + formatDate(posts[i].time.toString()/1000000) + "<br><br>";
